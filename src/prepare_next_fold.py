...
def main(args):
    ...
    for policy in ['guess', 'yes_no', 'example']:
        ...
        for root, dirs, fs in os.walk(policy_dir):
            user_source_dir = os.path.join(root, "source")
            if os.path.isdir(user_source_dir):
                ...
                user_predicates[int(root)] = all_user_predicates
                user_labels[int(root)] = all_user_labels

                print "user " + root + ":"
                print "\tnew preds: " + str(user_predicates[int(root)])
                print "\tnew labels: " + str(user_labels[int(root)])
